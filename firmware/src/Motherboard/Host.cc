...
void runHostSlice() {
  ...
  if (in.hasError()) {
    ...
    // Send a response saying you got an error.
    in.reset();
    out.reset();
    out.append8(RC_GENERIC_ERROR);
    out.append8(in.getErrorCode());
    UART::getHostUART().beginSend();
  }

  if (in.isFinished()) {
    Motherboard::getBoard().indicateError(0);
    packet_in_timeout.abort();
    out.reset();
  ...
  if (from_host.getLength() >= 1) {
    ...
    if ((command & 0x80) != 0) {
      ...
      case HOST_CMD_RECALL_HOME_POSITION:
        ...
        break;
      default:
        // command cannot be handled by this method.
        return false;
      }
...
sdcard::SdErrorCode startBuildFromSD() {
  ...
  return e;
#else
  return sdcard::SD_ERR_NO_CARD_PRESENT;
#endif
}
