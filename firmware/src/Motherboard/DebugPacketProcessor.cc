...
bool processDebugPacket(const InPacket& from_host, OutPacket& to_host) {
  ...
  if ((command & 0x70) == 0x70) {
    ...
    if (command == CommandCode::DEBUG_ECHO) {
      ...
      to_host.append8(RC_OK);
      char debug_message[DEBUG_MSG_BUFFER_SIZE];
      uint8_t len = Motherboard::getBoard().getDebugMessage(debug_message);
      to_host.append8(len);
      for (uint8_t i = 0; i < len; i++) {
        to_host.append8(debug_message[i]);
      }
      Motherboard::getBoard().setDebugMessage("", 0);
      return true;
    }
    return false;
  } else {
